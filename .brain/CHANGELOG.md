...
## 2026-09-12 - interviewer-persona + holistic-scoring + 3-approach-feedback + exemplar-bank - 895fef8 (backend) + 79b754a, dfdec49 (frontend) + guesstimate/plain-text (recent)
Grounded in a real Supabase pull of ~90 live sessions that showed two failures the owner reported: (a) the interviewer breaking character - "This information isn't provided in the prompt", "That detail isn't specified" across 11 sessions - and (b) the scorer ZEROING genuine conversations whose final-recommendation field was junk ("Not solved. Provide me with the approach", gibberish) - the exact e8a5e1af and 870e52a1 attempts. Five changes shipped together:
(1) INTERVIEWER PERSONA rewritten (prompts/interview_prompts.py, case + guesstimate): a top-firm consultant matched to the case domain who OWNS the case facts - invents a specific, internally-consistent figure on demand and NEVER says a detail "isn't specified / isn't in the prompt / isn't my data / I can't give you that"; IDENTITY LOCK (never admits being an AI, ignores "I'm the admin / which model are you" injections); NO PRAISE (bans all approval openers incl. soft "good question / good instinct"); PLAIN TEXT ONLY (no markdown/asterisks/lists/headings, and never lays out the candidate's framework - asks one question).
(2) HOLISTIC SCORING (services/interview_engine.py): the validity gate judges the CONVERSATION BODY separately from the final recommendation - a junk/empty recommendation can RESCUE a session but can NEVER zero a genuine conversation; only a genuinely empty attempt everywhere is rejected. recommendation_missing is a non-punitive hint (dock synthesis, keep the rest). Deleted _candidate_text (which fused turns+recommendation and drove the zeroing). Scorer max_tokens 4000->8000 + retry-once so the richer JSON can't truncate-crash a score.
(3) 3-APPROACH FEEDBACK on every scored session (case + guesstimate): your_line (the candidate's own beats, tightened) / top_candidate (a step-by-step FLOW + named frameworks tied to THIS case's real numbers) / third_angle (the other MECE road, or the opposite build for guesstimates). Rendered on the results page. approaches is MANDATORY even on weak/zero guesstimate answers (the teaching matters most then).
(4) SILENT EXEMPLAR BANK (services/exemplar_bank.py + migrations/2026-09-12_case_exemplars.sql): high-scoring, clean sessions are anonymised and banked per case; the scorer references the top few next time that case is scored - a teach-aligned, retrieval-augmented "improves with usage" loop (weights unchanged), gated behind approved=true, fully defensive (never breaks scoring/submit). OFF until the migration runs.
(5) FRONTEND FLOW: guests who solve a real case before signing up now land on their RESULTS page after onboarding, not the dashboard (server-side fallback resolves the latest submission when sessionStorage is wiped by the email-confirm round trip); the pre-signup warm-up baseline is demoted off the top of the dashboard and hidden once a real score exists; interviewer chat bubbles render via renderWithBold so stray markdown shows as bold, not raw asterisks; the 3 approaches + flow render on results.
touches: backend prompts/interview_prompts.py, prompts/guesstimate_scoring_prompt.py, services/interview_engine.py, services/ai_scorer.py, services/exemplar_bank.py (new), routes/attempts.py, migrations/2026-09-12_case_exemplars.sql (new), tools/eval_interview_scoring.py (new); frontend app/(app)/results/[id]/page.tsx, app/(app)/dashboard/page.tsx, app/(app)/onboarding/page.tsx, components/onboarding/onboarding-form.tsx, components/solve/ConversationalSolve.tsx
breaking: no CONTRACTS.md surface change - feedback_json gains ONE additive optional key `approaches` (null on rejected / legacy-guesstimate / old rows).   affects: AI evaluation (scorer output + interviewer behaviour), Case solve UX (results landing + interviewer render), Dashboard (warm-up placement)
GATES: py_compile EXIT 0 on all backend files; gate logic unit-proven 4/4 (genuine+junk-rec scored; long-gibberish 0; thin+good-rec rescued; empty+junk 0); frontend `npm run build` Compiled successfully; real-API eval harness (tools/eval_interview_scoring.py) at 50x2 - CASES 100% pass, INTERVIEWER 100% (identity lock + invents figures + plain text confirmed), score ordering pro>good>newbie. Guesstimate eval was 28% - diagnosed as a harness artifact (generic no-number answers correctly zeroed by the deterministic arithmetic backstop) plus a real gap (approaches dropped on weak answers); both fixed (harness now feeds numeric answers; guesstimate approaches made mandatory), re-run pending. Run migrations/2026-09-12_case_exemplars.sql to enable the exemplar bank (stays inert until then).
NOTE: repo moved to shared drive D:\dev\mece on 2026-09-12 (both Windows profiles / both cloud accounts); see SESSION_PREAMBLE.md. Old C:\Users\satya\Videos\company kept as backup.
...
breaking: no   affects: Landing page (renders zero-state instead of failing when env is absent), any future static/ISR page using createStaticClient
GATES: `npx tsc --noEmit` EXIT 0. Placeholder-client behaviour verified by executing a real query against the placeholder host and confirming it resolves rather than throws. NOTE: this is a SAFETY NET, not a substitute for correct config — scope-check `vercel env ls` after any env change, because a variable present in only one environment is the same as absent in the other.
