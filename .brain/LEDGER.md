...
|---|---|---|---|---|---|
| **Dashboard** | A | feat/dashboard | **Wired to live data + perf BUILT & LIVE (2026-06-08)** — constellation per-node mastery, cluster routing, branded loading skeletons, cached auth | `components/dashboard/*`, `lib/readiness.ts`, `lib/next-action.ts`, `lib/personal-stats.ts`, `components/dashboard-client.tsx` | DB:`cases`/`attempts`, Scoring-contract (reads `total`/`dimensions`) |
